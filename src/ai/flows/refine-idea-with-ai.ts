...
const refineIdeaFlow = ai.defineFlow(
  ...
  async (input: RefineIdeaInput): Promise<RefineIdeaOutput> => {
    ...
    if (textAnalysisOutput.refinedIdea) {
      try {
        const imageGenerationPromptText = `Generate a conceptual, visually appealing image that represents the following idea: ${textAnalysisOutput.refinedIdea}. The image should be suitable for a presentation or as a visual summary of the core concept.`;
