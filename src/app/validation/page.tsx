...
// Simplified chart config, as factors and their specific colors might not be predefined
// We'll use a single color for all bars from the theme.
const chartConfigBase = {
  score: {
    label: "Score (0-100)",
    color: "hsl(var(--chart-1))", // Default bar color
  },
  // Individual factors are now dynamically generated by AI
...
export default function ValidationPage(): ReactNode {
 ...
 useEffect(() => {
  ...
  const dynamicChartConfig = useMemo(() => {
    ...
    validationResult.viabilityFactorsChartData.forEach((factor, index) => {
      // Use a cycle of chart colors if more specific colors per factor are desired
      // For now, all bars will use the 'score' color defined in chartConfigBase
      // config[factor.name] = { label: factor.name, color: `hsl(var(--chart-${(index % 5) + 1}))` };
    });
    return config;
  }, [validationResult]);
      ...
      <div id="validation-results-section">
        ...
        {!isLoading && validationResult && (
          ...
          <Card className="mt-8 shadow-xl bg-gradient-to-br from-primary/10 via-card to-accent/10 border-accent/50">
            <CardHeader>
                <CardTitle className="font-headline text-2xl flex items-center text-primary">
                    <ShieldCheck size={28} className="mr-3" /> Premium AI Analysis
                </CardTitle>
                <CardDescription>AI-generated market insights and strategic advantages for your idea.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
            ...
            {/* Footer can be removed or simplified as it's no longer a "preview" with an upgrade button */}
